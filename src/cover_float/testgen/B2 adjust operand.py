...
def test_mul(fmt: str, desired_result: str, maxnorm: bool, sign: int, test_f: TextIO, cover_f: TextIO) -> None:
...
def test_div(fmt: str, desired_result: str, maxnorm: bool, sign: int, test_f: TextIO, cover_f: TextIO) -> None:
    ...
    if ans < desired_result:
        b = calibrate(a, 1) if sign == 0 else calibrate(b, -1)
    elif ans > desired_result:
        b = calibrate(a, -1) if sign == 0 else calibrate(b, 1)
            ...
            for base, (base_m, base_e) in bases.items():
                for i in range(m_bits):
                    ...
                    for sign in [0, 1]:
                        ...
                        test_mul(fmt, desired_result, maxnorm, sign, test_f, cover_f)
                        test_div(fmt, desired_result, maxnorm, sign, test_f, cover_f)
